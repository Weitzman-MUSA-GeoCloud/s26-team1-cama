...
def load_pwd_parcels(request):
    ...
    try:
        ...
        source_sql = read_sql("source_pwd_parcels.sql", replacements)
        core_sql = read_sql("core_pwd_parcels.sql", replacements)
        ...
        return (
            json.dumps(
                {
                    "ok": True,
                    "source_table": f"{project_id}.{source_dataset}.pwd_parcels",
                    "core_table": f"{project_id}.{core_dataset}.pwd_parcels",
                }
            ),
            200,
            {"Content-Type": "application/json"},
