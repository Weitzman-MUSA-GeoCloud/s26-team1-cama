...
def load_opa_properties(request):
    ...
    try:
        ...
        source_sql = read_sql("source_opa_properties.sql", replacements)
        core_sql = read_sql("core_opa_properties.sql", replacements)
        ...
        return (
            json.dumps(
                {
                    "ok": True,
                    "source_table": f"{project_id}.{source_dataset}.opa_properties",
                    "core_table": f"{project_id}.{core_dataset}.opa_properties",
                }
            ),
            200,
            {"Content-Type": "application/json"},
